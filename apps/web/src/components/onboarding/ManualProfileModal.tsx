...
export function ManualProfileModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150 overflow-y-auto">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-lg w-full p-5 sm:p-6 space-y-4 animate-in zoom-in-95 duration-150 relative my-4 max-h-[92vh] overflow-y-auto">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-primary-action/20 focus:border-primary-action"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-primary-action/20 focus:border-primary-action resize-none"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1.5 mb-1.5">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-primary-action/20 focus:border-primary-action"
              />
              <button
                type="button"
            ...
            {/* Selected Skill Tags */}
            {skills.length > 0 && (
              <div className="flex flex-wrap gap-1 p-1.5 rounded-xl bg-slate-50 border border-slate-200/80 mb-1.5 max-h-24 overflow-y-auto">
                {skills.map((skill) => (
                  <span
                    key={skill}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white border border-slate-200 text-[11px] font-semibold text-slate-800 shadow-2xs"
                  >
                    <span>{skill}</span>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    className={`px-1.5 py-0.5 rounded text-[10px] font-medium border transition-colors cursor-pointer ${
                      alreadyAdded
                        ...
                        : "bg-white text-slate-600 border-slate-200 hover:bg-primary-light hover:text-primary-action hover:border-primary-border"
                    }`}
                  >
                    +{s}
                  </button>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100">
            <button
              ...
              onClick={onClose}
              disabled={isSubmitting}
              className="px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
