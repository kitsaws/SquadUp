...
export function UniversityReminderModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-5 animate-in zoom-in-95 duration-150 relative">
        {/* Close icon button */}
        <button
          onClick={onClose}
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-3.5">
                {selectedUniversity.logoUrl ? (
                  <img
                    ...
                    className="w-11 h-11 rounded-xl object-contain bg-white border border-slate-200 p-1 shrink-0 shadow-2xs"
                  />
                ) : (
                  <div className="w-11 h-11 rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold text-sm shrink-0 font-heading shadow-2xs">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500">
                    ...
                    {selectedUniversity.domain && (
                      <span className="px-1.5 py-0.2 rounded bg-white text-slate-700 font-mono text-[10px] border border-slate-200">
                        @{selectedUniversity.domain}
                      </span>
                    )}
                  </div>
