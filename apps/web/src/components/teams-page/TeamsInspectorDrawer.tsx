...
interface TeamsInspectorDrawerProps {
  team: TeamCardData;
  isInline?: boolean;
  onClose: () => void;
  onApply: () => void;
...
export const getRoleMatchStatus = (
  ...
  for (const skill of skills) {
    if (checkSkillMatch(skill, team, isSignedIn, userVerifiedSkills)) {
      matchedCount++;
    }
  }
...
export function TeamsInspectorDrawer({
  team,
  isInline = false,
  onClose,
  onApply,
            ...
            <div className="space-y-2">
              {rolesList.length > 0 ? (
                rolesList.map((role, idx) => {
                  const matchStatus = getRoleMatchStatus(role, team, isSignedIn, userVerifiedSkills);
                  const isOptimalRole =
                    validBestRole?.roleTitle?.toLowerCase().trim() === role.title.toLowerCase().trim();
                  const roleSkills = role.skills || [];
                  const matchedCount = roleSkills.filter((s) => checkSkillMatch(s, team, isSignedIn, userVerifiedSkills)).length;
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-text-muted font-medium">
            {team.members.length} / {maxSpots} spots filled
          </span>
        </div>
